...
// Removed: import { useSettings } from '../context/SettingsContext';
...
import { parse as parseDate, isValid as isValidDate } from 'date-fns'; // Removed formatDateFns
...
const formatYMD = (date) => { if (!date) return ''; try { return new Date(date).toLocaleDateString('sv-SE'); } catch { return ''; } }; // _e removed

function Importar({ session }) {
    // Removed currency from useSettings()
    ...
    const parseAndValidateDate = (dateString) => { if (!dateString || typeof dateString !== 'string') return null; const fmts = ['yyyy-MM-dd', 'dd/MM/yyyy', 'MM/dd/yyyy', 'dd-MM-yyyy', 'MM-dd-yyyy', 'yyyy/MM/dd']; for (const fmt of fmts) { try { const p = parseDate(dateString, fmt, new Date()); if (isValidDate(p)) return formatYMD(p); } catch { /* Intentional: ignore parsing error for this format, try next */ } } return null; }; // _e removed from catch
