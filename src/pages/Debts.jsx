...
import {
  ...
  eliminarDebt,
  registrarPagoDeuda,
  obtenerTablaAmortizacion // <--- NUEVA IMPORTACIÓN
} from '../lib/debtsApi';
...
const formatYMD = (date) => {
  ...
  try {
    ...
    if (isNaN(d.getTime())) return ''; // Verificar si la fecha es válida
...
// Opciones predefinidas para los nuevos campos
const tiposDeudaOpciones = [
  'Personal', 'Tarjeta de Crédito', 'Hipotecario', 'Vehicular', 'Estudiantil', 'Otro'
];
const frecuenciasPagoOpciones = [ // Aunque la simulación actual es mensual
...
function Debts({ session }) {
  ...
  const [tasaInteres, setTasaInteres] = useState(''); // Tasa anual
  const [pagoMinimo, setPagoMinimo] = useState('');
  const [carteraPagoIdForm, setCarteraPagoIdForm] = useState('');
  const [fechaInicio, setFechaInicio] = useState('');
  const [notas, setNotas] = useState('');
  const [tipoDeuda, setTipoDeuda] = useState('Personal'); // <--- NUEVO ESTADO
  const [frecuenciaPago, setFrecuenciaPago] = useState('mensual'); // <--- NUEVO ESTADO
  const [plazoMeses, setPlazoMeses] = useState(''); // <--- NUEVO ESTADO para la amortización
  ...
  // Estados para Modal de Tabla de Amortización
  ...
  const [pagoSimuladoAmortizacion, setPagoSimuladoAmortizacion] = useState(''); // Para simular con un pago diferente
  ...
  const resetForm = () => {
    ...
    setTipoDeuda('Personal'); setFrecuenciaPago('mensual'); setPlazoMeses(''); // <--- RESET NUEVOS CAMPOS
  };
  ...
  const handleEditarClick = (debt) => {
    ...
    setTipoDeuda(debt.tipo_deuda || 'Personal'); // <--- SET NUEVO CAMPO
    setFrecuenciaPago(debt.frecuencia_pago || 'mensual'); // <--- SET NUEVO CAMPO
    // El plazo en meses no se guarda directamente en la deuda, se usa para simulación.
    // Podrías estimarlo si tienes fecha fin o calcularlo, pero por ahora se deja para entrada manual en simulación.
    setPlazoMeses('');
  ...
  const handleSubmit = async (event) => {
    ...
    const datosDeuda = {
      ...
      tipo_deuda: tipoDeuda, // <--- NUEVO CAMPO
      frecuencia_pago: frecuenciaPago, // <--- NUEVO CAMPO
    };
  ...
  // --- Lógica para Tabla de Amortización ---
  const openModalAmortizacion = (debt) => {
    setDeudaParaAmortizacion(debt);
    setPlazoMeses(''); // Limpiar plazo anterior
    setPagoSimuladoAmortizacion(debt.pago_minimo_mensual > 0 ? String(debt.pago_minimo_mensual) : ''); // Sugerir pago mínimo
    setTablaAmortizacion([]);
    setAmortizacionError('');
    setShowModalAmortizacion(true);
  };
  ...
  const handleCalcularAmortizacion = async () => {
    if (!deudaParaAmortizacion || !plazoMeses || parseInt(plazoMeses, 10) <= 0) {
      setAmortizacionError("Ingresa un plazo en meses válido."); return;
    }
    ...
    const tasaAnual = parseFloat(deudaParaAmortizacion.tasa_interes_anual) / 100; // Convertir % a decimal
    ...
    try {
      ...
      setAmortizacionError(`Error al calcular: ${err.message}`);
      setTablaAmortizacion([]);
    } finally {
      ...
      <section id="form-debt" className="bg-gray-900 p-6 rounded-lg shadow-lg">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div><label htmlFor="montoInicial" className={labelClasses}>Monto Inicial <span className="text-red-500">*</span></label><input type="number" id="montoInicial" value={montoInicial} onChange={(e) => setMontoInicial(e.target.value)} required min="0.01" step="0.01" className={inputClasses} disabled={loadingSettings}/></div>
            <div><label htmlFor="tasaInteres" className={labelClasses}>Interés Anual (%)</label><input type="number" id="tasaInteres" value={tasaInteres} onChange={(e) => setTasaInteres(e.target.value)} min="0" step="0.01" placeholder="Ej: 15 para 15%" className={inputClasses}/></div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div><label htmlFor="pagoMinimo" className={labelClasses}>Pago Mínimo Mensual</label><input type="number" id="pagoMinimo" value={pagoMinimo} onChange={(e) => setPagoMinimo(e.target.value)} min="0" step="0.01" className={inputClasses} disabled={loadingSettings}/></div>
            <div><label htmlFor="tipoDeuda" className={labelClasses}>Tipo de Deuda</label><select id="tipoDeuda" value={tipoDeuda} onChange={(e) => setTipoDeuda(e.target.value)} className={selectClasses}>{tiposDeudaOpciones.map(opt => <option key={opt} value={opt}>{opt}</option>)}</select></div>
            <div><label htmlFor="frecuenciaPago" className={labelClasses}>Frecuencia de Pago</label><select id="frecuenciaPago" value={frecuenciaPago} onChange={(e) => setFrecuenciaPago(e.target.value)} className={selectClasses}>{frecuenciasPagoOpciones.map(opt => <option key={opt.value} value={opt.value}>{opt.label}</option>)}</select></div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label htmlFor="carteraPagoIdForm" className={labelClasses}>Cartera Predeterminada (Opc)</label><select id="carteraPagoIdForm" value={carteraPagoIdForm} onChange={(e) => setCarteraPagoIdForm(e.target.value)} className={selectClasses} disabled={cargando}><option value="">-- Ninguna --</option>{cargando ? <option>Cargando...</option> : carteras.map(c => <option key={c.id} value={c.id}>{c.nombre}</option>)}</select></div>
            <div><label htmlFor="fechaInicioDeuda" className={labelClasses}>Fecha Inicio (Opc)</label><input type="date" id="fechaInicioDeuda" value={fechaInicio} onChange={(e) => setFechaInicio(e.target.value)} className={inputClasses} /></div>
          </div>
          <div><label htmlFor="notasDeuda" className={labelClasses}>Notas (Opc)</label><input type="text" id="notasDeuda" value={notas} onChange={(e) => setNotas(e.target.value)} className={inputClasses} /></div>
          <div className="flex space-x-3 pt-2"> <button type="submit" className={buttonClasses(editandoDebt ? 'yellow' : 'green')} disabled={isSubmitting || loadingSettings || cargando}> {isSubmitting ? 'Guardando...' : (editandoDebt ? '💾 Guardar' : '➕ Agregar')} </button> {editandoDebt && (<button type="button" onClick={handleCancelarEdicion} className={buttonClasses('gray')}> Cancelar </button>)} </div>
        </form>
                    ...
                    <tr key={debt.id} className="bg-gray-800 border-b border-gray-700 hover:bg-gray-600">
                      ...
                      <td className={`${tableCellClasses} text-center`}>
                        <div className="flex flex-col space-y-1 sm:space-y-0 sm:flex-row justify-center items-center sm:space-x-1">
                          ...
                          <button onClick={() => handleEliminarClick(debt.id)} className={`${actionButtonClasses} text-red-500 w-full sm:w-auto`} title="Eliminar Deuda">🗑️ Del</button>
                        </div>
                      </td>
                    </tr>
                  );
